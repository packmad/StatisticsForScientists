...
Degrees of freedom (df) appear in outputs such as $t(24.48) = -8.38$ and are used to determine $p$-values and confidence intervals. Conceptually, the degrees of freedom represent the amount of independent information available to estimate variability. In simple settings, this is closely related to sample size, but it is reduced by the need to estimate quantities such as group means or variances.

In Welch's $t$-test (used throughout this tutorial for mean differences), the degrees of freedom are not necessarily an integer. This is because the method adjusts for unequal variances between groups, producing an \emph{effective} degrees of freedom that reflects both sample sizes and variability. Smaller df correspond to greater uncertainty and heavier tails in the reference distribution, leading to wider confidence intervals and more conservative inference.
...
In a hypothesis test we choose a \emph{test statistic} \(T(\mathbf{X})\): a single number computed from the data \(\mathbf{X}\) that would tend to be large (or small) when the null hypothesis \(H_0\) is false. After observing \(t_{\text{obs}} = T(\mathbf{x}_{\text{obs}})\), the \emph{p-value} is the probability, \emph{assuming \(H_0\) and the rest of the model are true}, of getting a test statistic at least as extreme as what was observed. For a two-sided test this is commonly written as
\[
 p = \Pr\!\left(|T(\mathbf{X})| \ge |t_{\text{obs}}| \mid H_0\right),
\]
where ``extreme'' is defined with respect to the null distribution of \(T(\mathbf{X})\) (for example a \(t\) distribution for Welch's \(t\)-test, or the null distribution used for \(U\) in a Mann--Whitney test). For a one-sided test, the tail is taken in the direction specified by the alternative hypothesis. The p-value is \emph{not} the probability that \(H_0\) is true; it is a statement about how surprising the observed data would be \emph{if} \(H_0\) were true.
